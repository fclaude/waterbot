...
def load_schedules():
    ...
    if os.path.exists(SCHEDULE_CONFIG_FILE):
        try:
            ...
            print(f"Warning: Could not load schedule config file {SCHEDULE_CONFIG_FILE}: {e}")
...
def remove_schedule(device, action, time):
    ...
    Returns:
        bool: Success status
    """
    if (device in DEVICE_SCHEDULES and 
        action in DEVICE_SCHEDULES[device] and 
        time in DEVICE_SCHEDULES[device][action]):
